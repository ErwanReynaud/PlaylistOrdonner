...
| `403` sur **toutes** tes playlists alors que les titres likés passent | Regarde la ligne « Pays du compte » de `python3 -m playlistordonner tester` : s'il est inconnu, l'autorisation `user-read-private` manque. Spotify ne peut alors pas déterminer ton marché et refuse de lister les titres. Reconnecte-toi pour l'accorder. |
| `403` sur toutes tes playlists malgré un pays connu | Vérifie-le avec `python3 -m playlistordonner tester` (sans argument) : si le compte et les titres likés répondent alors qu'aucune playlist n'est lisible, c'est ton application Spotify qui est restreinte. Crée-en une nouvelle sur le dashboard, puis clique sur « Changer d'application Spotify » pour saisir son Client ID. |
